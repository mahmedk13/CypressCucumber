...
const cucumberJsonDir = path.resolve(process.cwd(), "cypress/cucumberJson");
const cucumberReportFileMap = {};
const cucumberReportMap = {};
const jsonIndentLevel = 2;
const htmlReportDir = path.resolve(process.cwd(), "cypress/cucumberJson");
...
function generateReport() {
  if (!fs.existsSync(cucumberJsonDir)) {
    ...
    report.generate({
      ...
      useCDN: true,
      pageTitle: "Cypress Framework",
      reportName: `Cypress Framework - ${new Date().toLocaleString()}`,
      metadata: {
        app: {
          name: "CPARS",
          version: "1",
